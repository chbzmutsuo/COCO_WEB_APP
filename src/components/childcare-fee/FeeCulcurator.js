...
export default function FeeCulcurator() {
	...
	const culcurate = (e) => {
		...
		if (!(exception && childType)) { alert("正しく乳力をしてください"); return }
		if (exception === '0' && !nenshu) { alert("正しく乳力をしてください"); return }
	...
	return (
		...
		<div>
			<Form className='flex flex-wrap  max-w-md mx-auto' >
				...
				<div className='w-full p-1'>
					<FloatingLabel controlId="floatingSelect" label="次のいずれかに当てはまりますか?">
						<Form.Select aria-label="Floating label select example" value={exception} onChange={onChangeException} style={{ height: 80, overflowWrap: "break-all" }} >
							<option value="">選択してください</option>
							<option value="0">該当しない</option>
							<option value="A">A	被保護世帯及び中国残留邦人等の円滑な帰国の促進及び永住帰国後の自立の支援に関する法律による支援給付受給世帯</option>
			...
			<BtnBasic center onClick={culcurate}>再計算</BtnBasic>
